...
int userdata_save(uint8_t entry)
{
	...
	if (entry > MAX_PROFILE)
	{
		...
		for (int i = 0; i < (sizeof(initcfg) + 3) / 4; i++) // We might save some trash, but it doesn't matter...
...
int userdata_load(uint8_t entry, int dry_run)
{
	...
	ude_hdr *hdr = (void *)(FLASH_MEM_BASE + USERDATA_OFFSET + entry * USERDATA_ENTRY_SIZE);
	...
	switch (hdr->type)
	{
		case UDE_INITCFG:
		{
			if (hdr->version_major != INITCFG_VER_MAJOR || hdr->version_minor != INITCFG_VER_MINOR)
			{
		...
		case UDE_PROFILE:
			if (hdr->version_major != PROFILE_VER_MAJOR || hdr->version_minor != PROFILE_VER_MINOR) {
...
int userdata_export()
{
    ...
    uint32_t btn_vec, sd_block_offset;
...
eval_button:
    ...
    for (uint32_t sector = 0;
        ...
        ++sector)
    {
		retval = sdcard_write(sector * SD_BLK_SIZE, databuf, SD_BLK_SIZE);
		if (retval)
            goto out;
    }
...
copy_start:
	...
	retval = sdcard_write(sd_block_offset * SD_BLK_SIZE, (void *)(FLASH_MEM_BASE + USERDATA_OFFSET), (MAX_USERDATA_ENTRY + 1) * USERDATA_ENTRY_SIZE);
